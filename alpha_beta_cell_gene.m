function [alphabeta_c, alphabeta_g] = alpha_beta_cell_gene(X, A, fig_nr)

% Kajsa Mollersen (kajsa.mollersen@uit.no) 22nd October 2018

% Similar to the alpha_beta_cell function, but here including the gene effect
...
% Cell effect: There are n cells, each with different alpha and beta. 
...
% Gene effect: There are d genes, each with different alpha and beta. 

% We assume that 0 < pi < 1, and therefore set the minimum count to 1. 
...
t_g = zeros(d,2);
for j = 1 : d
  n00 = max(sum(~X(:,j) & ~ A(:,j)),1); % Let it be minimum 1 to avoid 1/0
  n01 = max(sum(~X(:,j) & A(:,j)),1); % Let it be minimum 1 to avoid 1/0
  n10 = max(sum(X(:,j) & ~A(:,j)),1); % Let it be minimum 1 to avoid log 0
  n11 = max(sum(X(:,j) & A(:,j)),1);  % Let it be minimum 1 to avoid log 0
  alpha_g(j) = log(n10/(n00+eps) + eps);
  beta_g(j) = log(n11/(n01+eps) + eps) - log(n10/(n00+eps) + eps);
  pi0_g(j) = n10/(n00+n10);% exp(alpha_c(i))/(1 + exp(alpha_c(i))); 
  pi1_g(j) = n11/(n01+n11);% exp(alpha_c(i) + beta_c(i))/(1 + exp(alpha_c(i)+ beta_c(i))); 
  t_g(j,1) = log((n00+n10)/(n00+eps) + eps);
  t_g(j,1) = log((n01+n11)/(n01+eps) + eps);
end

alphabeta_g = [alpha_g; beta_g; t_g; pi1_g];
...
for i = 1 : n
  for j = 1 : d
    Pi(i,j) = pi0_c(i) + pi0_g(j) + (pi1_c(i) - pi0_c(i))*A(i,j);
  end
end
